...
public abstract class IronSubsystem extends SubsystemBase {
  ...
  /**
   * Returns a command to print the message out with a timestamp in {@linkplain System.out standard
   * out} Also sends a notification to elastic with the same message
   *
  ...
  /**
   * DEBUGGING ONLY Publish a Sendable (including Commands) diagnostic value to SmartDashboard with
   * the prefix `debug/Subsystems/{subsystem name}/`. If the value is a Command, it will also be
   * registered with PathPlanner's {@link NamedCommands}.
   */
